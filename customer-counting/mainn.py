...
def save_statistics(entry_count, exit_count):
    ...
    df.to_csv('outputs/statistics.csv', mode='a', header=not os.path.exists('outputs/statistics.csv'), index=False)
...
def view_statistics():
    ...
    try:
        ...
        st.error("Không tìm thấy file lịch sử. Hãy đảm bảo bạn đã chạy chế độ ghi dữ liệu!")
...
if __name__ == "__main__":
    ...
    mode = st.sidebar.radio("Chế độ hoạt động", ["Ghi dữ liệu", "Xem lại lịch sử"])

    if mode == "Ghi dữ liệu":
        st.sidebar.warning("Hãy tắt Streamlit và chạy trực tiếp file Python để ghi dữ liệu!")
    elif mode == "Xem lại lịch sử":
        view_statistics()
